...
class FishingView( discord.ui.View ):
    ...
    async def __get_embed( self, position:str ) -> discord.Embed:
        ...
        t_embed = discord.Embed( title = '釣魚占卜', color = discord.Colour.from_rgb( 222, 199, 241 ) )
        ...
        t_embed.add_field( name = '🍀 幸運值', value = f'({lucky_degree}) {lucky_status}', inline = True )
...
class FishingWishForm( discord.ui.Modal ):
    def __init__( self, *args, **kwargs ) -> None:
        ...
        self.add_item( discord.ui.InputText( label = '請填寫<解析>', style = discord.InputTextStyle.long ) )
    # __init__()
    ...
    async def callback( self, interaction: discord.Interaction ):
        ...
        if not self.__success_goto_process():
            embed = discord.Embed( description = '您的許願卡不幸被海怪吞了..下次再來吧!',
        ...
        # if
            ...
            if not success:
                embed = discord.Embed( description = reason, color = discord.Colour.from_rgb( 0, 255, 255 ) )
            # if
            else:

                embed = discord.Embed( description = '您的許願卡已投入河中..',
                                    color = discord.Colour.from_rgb( 0, 255, 255 ) )
                embed.set_author( name = 'EB Extend BOT',
                                  url = 'https://twitter.com/0xmimiQ',
                                  icon_url = 'https://i.imgur.com/DmV9HWw.png' )
                
                embed.add_field( name = '🆕 許願序號', value = seq, inline = False )
                embed.add_field( name = '🎭 物種名稱', value = self.children[0].value, inline = False )
                embed.add_field( name = '🖊️ 解析', value = self.children[1].value, inline = False )
            # else
        # else
