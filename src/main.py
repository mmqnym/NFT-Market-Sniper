...
def main() -> None:
    ...
    if success:
        ...
        if (
            ...
            or CFG.get_owner_id() == 0
            or CFG.get_system_log_channel() == 0
            or CFG.get_cronoscan_api_key() == "Your Key"
            or CFG.get_cmc_api_key() == "Your Key"
        ):
            MAIN_LOGGER.critical(
                "請先在 src/configs.json 中寫入"
...
def make_embed(
    ...
    more_fields: Optional[List[list]] = None,
) -> "discord.Embed":
    """
    ...
    if (is_local_image and "attachment://" not in image_url) and file is None:
        raise Exception(
            "Local image flag is Set, but not match format or file is not submitted."
        )
...
@bot.slash_command(
    name="bind",
    description="Bind a channel to send system / mint status / floor price info.",
)
@commands.is_owner()
...
async def bind(
    ...
    channel: Option(discord.TextChannel, "Choose a channel"),
) -> None:
    ...
    if not success:
        ...
        embed = make_embed(
            description=f"推送頻道 [{type}] 已綁定: {channel.mention}",
            color=dc_color("green"),
    ...
    for job in jobs:
        ...
        if i % 3 == 0:
            ...
            t_embed = discord.Embed(
                title="", description="當前所有任務清單", color=dc_color("purple")
            )
        # if
    # for
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def list_jobs(
    ctx: discord.ApplicationContext, user: Option(discord.User, "Tag a person.")
) -> None:
    ...
    t_embed = discord.Embed(
        title="", description=f"{user.mention} 的任務清單", color=dc_color("purple")
    )
    jobs = SCHEDULER.get_jobs()
    ...
    for job in jobs:
        if source_id == job.owner_id:
            ...
            series = (job.eb_url).replace("https://app.ebisusbay.com/collection/", "")
            t_embed.add_field(
                name="目標系列", value=f"[{series}]({job.eb_url})", inline=True
            )
            t_embed.add_field(name="當前地板價", value=f"`{job.cur_floor} CRO`", inline=True)
            t_embed.add_field(name="通知對象", value=job.mention_target, inline=True)
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def set_floor_tracker(
    ctx: discord.ApplicationContext,
    url: Option(str, "Enter a Ebisu's bay collection url."),
) -> None:
    ...
    if CFG.get_floor_tracker_channel() == 0:
        embed = make_embed(
            description=f"您必須先設定地板價變動發送頻道 參考：`/bind`",
            color=dc_color("red"),
            thumbnail_url=CFG.get_error_img_url(),
        )
        await ctx.respond(embed=embed)
    ...
    # if
        ...
        embed = make_embed(
            description="檢驗是否為存在的 Ebisu's bay 收藏，請稍後...", color=dc_color("purple")
        )
        ...
        # if
            ...
            embed = make_embed(
                description=ctx.author.mention + " 請TAG您想通知的身分組或個人",
                color=dc_color("purple"),
            )
            await sent_msg.edit_original_message(embed=embed)

            usr_answer = await bot.wait_for(
            ...
            if ("<@&" == mention_target[0:3] and ">" == mention_target[-1]) or (
                ...
                (success, reason) = SCHEDULER.add_job(
                    ctx.author.id, erc_type, url, floor_price, mention_target
                )
                ...
                # if
                    ...
                    t_list = [
                        ["任務擁有者", ctx.author.mention, False],
            ...
            # if
                ...
                embed = make_embed(
                    description="錯誤的tag參數，請確認您tag的是否為身份組或個人",
                    color=dc_color("red"),
                    thumbnail_url=CFG.get_error_img_url(),
                )
                await sent_msg.edit_original_message(embed=embed)
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def delete_floor_tracker(
    ctx: discord.ApplicationContext,
    url: Option(str, "Enter a Ebisu's bay collection url which in the record."),
) -> None:
...
@commands.is_owner()
@commands.cooldown(1, 15, commands.BucketType.user)
async def set_mint_tracker(
    ctx: discord.ApplicationContext,
    address: Option(str, "Enter contract address of a collection."),
    token_name: Option(str, "Give this token a name."),
    total_supply: Option(
        str,
        "Give this token its total supply."
        + "You can use - instead if you don't need to know it.",
    ),
) -> None:
    ...
    if CFG.get_mint_tracker_channel() == 0:
        embed = make_embed(
            description="您必須先設定鑄幣變動發送頻道 參考：`/bind`",
            color=dc_color("red"),
            thumbnail_url=CFG.get_error_img_url(),
        )
        await ctx.respond(embed=embed)
    ...
    if cur_supply == total_supply:
        embed = make_embed(
            description="本合約地址已達最大 mint 數，追蹤設置無效。",
            color=dc_color("red"),
            thumbnail_url=CFG.get_error_img_url(),
        )
        await ctx.respond(embed=embed)
    ...
    (success, reason) = SCHEDULER.add_mint_tracker(
        address, token_name, total_supply, cur_supply
    )
...
async def list_all_mint_trackers(ctx: discord.ApplicationContext) -> None:
    ...
    t_embed = discord.Embed(
        title="", description="當前所有鑄幣追蹤清單", color=dc_color("purple")
    )
...
async def choose(
    ctx: discord.ApplicationContext,
    *,
    options: Option(str, "Enter your options separated by , or space."),
) -> None:
    ...
    embed = make_embed(
        description=choices_title,
        color=dc_color("purple"),
        thumbnail_url=CFG.get_choose_img_url(),
        more_fields=[["選擇", choice, False]],
    )
    await ctx.respond(embed=embed)
...
@bot.slash_command(
    name="fishing", description="Go to bay and test your luck today by fishing."
)
...
async def fishing(ctx: discord.ApplicationContext) -> None:
    ...
    embed = make_embed(
        title="釣魚占卜 :fishing_pole_and_fish:",
        description="選擇您想前往釣魚占卜的地方 :arrow_right:",
        color=dc_color("purple"),
        thumbnail_url=CFG.get_choose_img_url(),
    )
...
@bot.slash_command(
    name="wish", description="Wishing for the birth of new fish in the bay."
)
@commands.cooldown(1, 72000, commands.BucketType.user)  # will change cd to 22 hr
...
async def status(ctx: discord.ApplicationContext):
    ...
    service_status = (
        cronos_api_status
        and cmc_api_status
        and track_mint_status.is_running()
        and track_floor_price.is_running()
    )
    ...
    embed = make_embed(
        title=f"響應時間: `{int( bot.latency * 1000 )} ms`",
        description=desc,
        color=status_color,
...
@sys.command(
    name="start_mint_tracker_task", description="To execute mint tracker task."
)
@commands.is_owner()
async def start_mint_tracker_task(ctx: discord.ApplicationContext):
    ...
    embed = make_embed(
        description=desc, color=color, thumbnail_url=CFG.get_done_img_url()
    )
    await ctx.respond(embed=embed)
...
@sys.command(
    name="stop_mint_tracker_task", description="Stop executing mint tracker task."
)
@commands.is_owner()
...
@sys.command(
    name="start_floor_tracker_task", description="To execute floor tracker(job) task."
)
@commands.is_owner()
...
@sys.command(
    name="stop_floor_tracker_task", description="Stop executing mint tracker task."
)
@commands.is_owner()
...
@commands.cooldown(1, 15, commands.BucketType.user)
async def convert_cro(
    ctx: discord.ApplicationContext,
    cro: Option(float, "Enter a value in CRO you want."),
) -> None:
    ...
    if cro < 0:
        ...
        embed = make_embed(
            description="`0 CRO` <:arrow:1002316255736369192> `0 USD`",
            color=dc_color("l_blue"),
        )
        ...
        if price == -1:
            embed = make_embed(
                description="無法從 CMC API 獲取最新價格資訊，請使用 `/sys status` 檢查狀態",
                color=dc_color("red"),
            )
        # if
        else:
            price *= cro
            embed = make_embed(
                description=f"`{cro} CRO` <:arrow:1002316255736369192> `{price} USD`",
                color=dc_color("l_blue"),
            )
        # else
    # else
...
async def convert_usd(
    ctx: discord.ApplicationContext,
    usd: Option(float, "Enter a value in USD you want."),
) -> None:
    ...
    if usd < 0:
        ...
        embed = make_embed(
            description="`0 USD` <:arrow:1002316255736369192> `0 CRO`",
            color=dc_color("l_blue"),
        )
        ...
        # if
            ...
            price *= usd
            embed = make_embed(
                description=f"`{usd} USD` <:arrow:1002316255736369192> `{price} CRO`",
                color=dc_color("l_blue"),
            )
        # else
    # else
...
async def track_mint_status():
    ...
    for tracker in trackers:
        contract_addr = tracker.contract_addr
        cur_supply = TRACKER.current_token_supply(contract_addr)

        if cur_supply == "":
            error_times_on_tm += 1
            MAIN_LOGGER.warning(f"暫時無法取得:{contract_addr} 此合約資訊!")
        ...
        if new_supply_int > old_supply_int:
            ...
            (success, reason) = SCHEDULER.update_tracker(
                tracker.contract_addr, new_cur_supply
            )
            ...
            if not success:
                ...
                track_mint_status.stop()
                embed = make_embed(
                    description="Mint 數量無法更新至資料庫，任務已終止。",
                    color=dc_color("red"),
                    thumbnail_url=CFG.get_error_img_url(),
                )
                await channel.send(embed=embed)
            ...
            embed = make_embed(
                ...
                more_fields=[
                    ["名稱", f"`{tracker.token_name}`", False],
                    ["最大供應", f"`{tracker.total_supply}`", False],
                    [
                        "供應變化",
                        f"`{old_cur_supply}` <:arrow:1002316255736369192> "
                        + f"`{new_cur_supply}`",
                        False,
                    ],
                ],
...
async def track_floor_price():
    ...
    for job in jobs:
        (screenshot_path, new_floor_price) = await TRACKER.track_with_detail(
            job.type, job.eb_url
        )
        ...
        if new_floor_price != "" and new_floor_price != job.cur_floor:
            ...
            if job.type == "721":
                ...
                embed = make_embed(
                    ...
                    more_fields=[
                        [
                            "地板價變化",
                            f"`{old_cur_floor}` <:arrow:1002316255736369192> "
                            + f"`{job.cur_floor}`",
                            False,
                        ]
                    ],
                    is_local_image=True,
                    image_url=f"attachment://{screenshot_path}",
                )
                view = TrackerMsgView()
                check_button = discord.ui.Button(
                    label="Check", style=discord.ButtonStyle.link, url=job.eb_url
                )
                view.add_item(check_button)
                await channel.send(
                    content=f"地板價更新通知! {job.mention_target}",
                    file=file,
                    embed=embed,
                    view=view,
                )
            # if
                ...
                series = (job.eb_url).replace(
                    "https://app.ebisusbay.com/collection/", ""
                )
                embed = make_embed(
                    title="地板價更新! (ERC1155) 🔥🔥🔥",
                    color=dc_color("l_blue"),
                ...
                view.add_item(check_button)
                await channel.send(
                    content=f"地板價更新通知! {job.mention_target}", embed=embed, view=view
                )
            # else
        # if
