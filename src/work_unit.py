...
class Job:
    def __init__( self, owner_id:int = 0, type:str = '', eb_url:str = '', cur_floor:str = '', 
...
class Jobs:
    ...
    def new_job( self, owner_id:int, type:str, eb_url:str, cur_floor:str, mention_target:str ) -> Job:
...
class MintTracker:
    def __init__( self, contract_addr:str = '', token_name:str = '', total_supply:str = '', cur_supply:str = '0' ) \
                  -> None:
...
class MintTrackers:
    ...
    def new_tracker( self, contract_addr:str = '', token_name:str = '', total_supply:str = '', cur_supply:str = '0' ) \
                     -> MintTracker:
...
class SqlDB:
    ...
    def insert_job( self, job:Job ) -> Tuple[bool, str]:
        ...
        sql = ( 'INSERT INTO jobs ( OWNER_ID, TYPE, EB_URL, CUR_FLOOR, MENTION_TARGET ) VALUES ' +
    ...
    def insert_mint_tracker( self, tracker:MintTracker ) -> Tuple[bool, str]:
        ...
        sql = ( 'INSERT INTO mint_trackers ( TOKEN_NAME, CONTRACT_ADDR, TOTAL_SUPPLY, CUR_SUPPLY ) ' +
    ...
    def update_specific_mint_tracker( self, contract_addr:str, new_cur_supply:str ) -> Tuple[bool, str]:
        '''
        ...
        sql = ( f'UPDATE mint_trackers SET CUR_SUPPLY = \'{new_cur_supply}\' ' +
    ...
    def fetch_ramdom_record_east( self ) -> Tuple[bool, str, dict]:
        ...
        sql = ( 'SELECT * FROM `fishing_e` WHERE id >= ((SELECT MAX(id) FROM fishing_e)' +
                '- (SELECT MIN(id) FROM fishing_e)) * RAND() + (SELECT MIN(id) FROM fishing_e) LIMIT 1' )

        try:
            cursor.execute( sql )
            record = cursor.fetchall()[0]
            result = { 'ID':f'E.{record[0]}', 'NAME':record[1], 'LUCKY':record[2], 'DESCRIPTION':record[3] }
            return ( True, '', result )
        # try
    ...
    def fetch_ramdom_record_west( self ) -> Tuple[bool, str, dict]:
        ...
        sql = ( 'SELECT * FROM `fishing_w` WHERE id >= ((SELECT MAX(id) FROM fishing_w)' +
                '- (SELECT MIN(id) FROM fishing_w)) * RAND() + (SELECT MIN(id) FROM fishing_w) LIMIT 1' )

        try:
            cursor.execute( sql )
            record = cursor.fetchall()[0]
            result = { 'ID':f'W.{record[0]}', 'NAME':record[1], 'LUCKY':record[2], 'DESCRIPTION':record[3] }
            return ( True, '', result )
        # try
    ...
    def fetch_ramdom_record_south( self ) -> Tuple[bool, str, dict]:
        ...
        sql = ( 'SELECT * FROM `fishing_s` WHERE id >= ((SELECT MAX(id) FROM fishing_s)' +
                '- (SELECT MIN(id) FROM fishing_s)) * RAND() + (SELECT MIN(id) FROM fishing_s) LIMIT 1' )

        try:
            cursor.execute( sql )
            record = cursor.fetchall()[0]
            result = { 'ID':f'S.{record[0]}', 'NAME':record[1], 'LUCKY':record[2], 'DESCRIPTION':record[3] }
            return ( True, '', result )
        # try
    ...
    def fetch_ramdom_record_north( self ) -> Tuple[bool, str, dict]:
        ...
        sql = ( 'SELECT * FROM `fishing_n` WHERE id >= ((SELECT MAX(id) FROM fishing_n)' +
                '- (SELECT MIN(id) FROM fishing_n)) * RAND() + (SELECT MIN(id) FROM fishing_n) LIMIT 1' )

        try:
            cursor.execute( sql )
            record = cursor.fetchall()[0]
            result = { 'ID':f'N.{record[0]}', 'NAME':record[1], 'LUCKY':record[2], 'DESCRIPTION':record[3] }
            return ( True, '', result )
        # try
...
class Scheduler:
    ...
    def add_job( self, ctx_author_id:int, type:str, eb_url:str, floor_price:str, mention_target:str ) \
                 -> Tuple[bool, str]:
        '''
        Add job to job list, if there is no collision.\n
        return ( success, reason )
        ...
        if ( owner_id != -1 ):
            reason = ( f'已存在相同追蹤目標，請聯繫任務擁有者 <@{owner_id}> ' +
                       '考慮是否需要合併任務，如您即為擁有者，想變更設定，請使用 `/delete [url]` 先刪除任務' )
            return ( False, reason )
        # if

        job = self.__job_controller.new_job( ctx_author_id, type, eb_url, floor_price, mention_target )
    ...
    def add_mint_tracker( self, contract_addr:str, token_name:str, total_supply:str, cur_supply:str ) \
                          -> Tuple[bool, str]:
        '''
        Add tracker to mint tracker list, if there is no collision.\n
        return ( success, reason )
        ...
        tracker = self.__mint_controller.new_tracker( contract_addr, token_name, total_supply, cur_supply )
    ...
    def update_tracker( self, contract_addr:str, new_cur_supply:str ) -> Tuple[bool, str]:
        '''
        Update tracker current supply in mint tracker list.\n
        return ( success, reason )
        '''
        ...
        ( success, reason ) = self.__db.update_specific_mint_tracker( contract_addr, new_cur_supply )
